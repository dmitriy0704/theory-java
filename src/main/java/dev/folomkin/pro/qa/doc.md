...
Stack Memory
└─ main()
   └─ ссылка на объект (MyObject@1234)

Heap Memory
└─ MyObject instance (value=0)
```
...
## 🔥 Бонус: интересные детали

- **Metaspace** — отдельная память для метаданных классов (с Java 8).
- **String Pool** — отдельная область для интернированных строк (строки без
  `new`).
- Разные сборщики мусора (`G1GC`, `ZGC`, `Shenandoah`) оптимизируют работу с
  кучей по-разному.

---

Хочешь, ещё могу рассказать:

- 🔹 Как устроены **разные типы сборщиков мусора** для кучи (например, G1GC как
  делит кучу на регионы),
- 🔹 Или показать, как **визуально** Heap выглядит в профайлерах типа VisualVM?

Хочешь продолжить? 🚀✨  
Очень классная тема для глубокого понимания Java-производительности!
...
_Молодое поколение (Young Generation)_
...
Отличный вопрос! 🔥  
Давай аккуратно и понятно разберем, **что такое стек памяти в Java**, зачем он
нужен и как работает.

---

## 📚 Что такое стек памяти (Stack Memory) в Java?
...
- 🔹 Как JVM оптимизирует стек через **инлайн методов**,
- 🔹 Или как настраивать **размер стека** при старте приложения (`-Xss`)?

Продолжим? 🚀✨  
Тема про стек реально открывает глаза на производительность Java-кода!
...
### Типы ссылок в памяти

Отличный вопрос! 😎  
В Java реально есть **разные типы ссылок** на объекты в памяти, и это очень
важно для управления памятью, особенно когда дело касается **сборки мусора** (
Garbage Collection).  
Погнали по порядку:
...
| Тип ссылки            | Описание                               | Когда объект собирается мусором                       |
|:----------------------|:---------------------------------------|:------------------------------------------------------|
| **Strong Reference**  | Обычная сильная ссылка.                | Никогда, пока есть хотя бы одна ссылка.               |
| **Soft Reference**    | Мягкая ссылка (`SoftReference`).       | При нехватке памяти.                                  |
| **Weak Reference**    | Слабая ссылка (`WeakReference`).       | При следующем проходе GC.                             |
| **Phantom Reference** | Фантомная ссылка (`PhantomReference`). | После финализации объекта, перед удалением из памяти. |
...
Это обычная работа Java:

```java
String s = "Hello";
```

- Пока `s` указывает на объект `"Hello"`, **GC не удалит его**.
- Чтобы объект стал доступен для сборки, **надо убрать все сильные ссылки**.

---

### 2. **Soft Reference** — Мягкая ссылка

Используется для кэширования, например:

```java
SoftReference<String> softRef = new SoftReference<>(new String("Hello"));
```

- Если **мало памяти**, **GC может удалить** объект, на который указывает
  `softRef`.
- Если памяти хватает, объект останется в памяти.

**Пример использования:**  
Кеши, которые хочется сбрасывать только при нехватке ресурсов.
...
- Объект будет удалён **при ближайшем сборке мусора**, даже если в системе
  достаточно памяти.
- Очень легко исчезает.

**Пример использования:**

- `WeakHashMap` (ключи удаляются, если больше нет сильных ссылок на них).
- Слушатели событий, которые не должны блокировать сборку мусора.
...
- **Нельзя получить объект через `phantomRef.get()` — всегда `null`**.
- Используется, чтобы **отследить момент**, когда объект уже недоступен, и
  провести какие-то действия **перед реальным освобождением памяти** (например,
  освобождение нативных ресурсов).

**Пример использования:**

- Управление ресурсами вне Java-хипа, например, чистка файлов, соединений.
...
```
Сильная ссылка  --->  Объект в памяти (GC НЕ может убрать)
Мягкая ссылка    -?->
Слабая ссылка    -??->
Фантомная ссылка -???->
```

- Чем **слабее** ссылка, тем **раньше** объект может быть удалён сборщиком
  мусора.
- **Фантомная** самая "тонкая" — она даже не даёт получить сам объект!

---

## 🎯 Выжимка:

| Тип         | Когда объект удаляется    |
|-------------|---------------------------|
| **Strong**  | Никогда, пока есть ссылка |
| **Soft**    | При нехватке памяти       |
| **Weak**    | При следующем GC          |
| **Phantom** | После финализации         |

---

Хочешь, ещё могу показать на **реальных примерах кода**:  
🔹 как использовать `WeakHashMap`,  
🔹 или как сделать **кэш с SoftReference** для картинок или файлов? 🎯
...
Если объект достижим из GC Root — он считается **живым**.

**Как работает GC в куче?**

Куча (Heap) делится на:

- **Young Generation** (молодое поколение)
- **Old Generation** (старое поколение)

И GC работает немного по-разному для них:

**Сборка мусора в Young Generation — *Minor GC***

- Быстрая и частая.
- Очищает пространство Eden + Survivor.
- Если объект пережил несколько сборок => переносится в Old Generation.

**Сборка мусора в Old Generation — *Major GC (или Full GC)***

- Редкая, но тяжелая.
- Очищает старые объекты в Old Gen.
- Может привести к **"стоп-свету"** (Pause the world) — приложение
  замораживается на время чистки.

**Какие алгоритмы используются?**

| Алгоритм             | Идея                                         | Где применяется    |
|----------------------|----------------------------------------------|--------------------|
| **Mark and Sweep**   | Пометить живые, удалить мертвые              | Базовый подход     |
| **Mark-Compact**     | Пометить, удалить, сжать память              | Old Gen            |
| **Copying**          | Копировать живые в новое место               | Eden (Young Gen)   |
| **Generational GC**  | Делит объекты на "молодые" и "старые"        | Везде по умолчанию |
| **G1 GC**            | Делит память на регионы, параллельная сборка | Большие кучи       |
| **ZGC / Shenandoah** | Сборка с минимальными паузами                | Супербольшие кучи  |

**Как выглядит жизнь объекта?**

```plaintext
1. Объект создается в Eden.
2. Если выжил после нескольких Minor GC — переходит в Old Gen.
3. Если объект в Old Gen больше не нужен — его убирает Major GC.
```

**Важно: Паузы в приложении**

Во время GC **может происходить пауза** в работе приложения:

- Minor GC — маленькая пауза
- Major GC — большая пауза
- Новые сборщики (G1, ZGC) стараются делать паузы минимальными

Когда может произойти ошибка OutOfMemoryError?

- Если куча полностью заполнилась, а очистка GC не помогает.
- Например:
    - Много объектов в памяти
    - Утечки памяти (`Memory Leak`)
    - Ошибки в коде (`static` коллекции растут бесконечно)

Схема работы GC:

```plaintext
Start -> [Mark] -> [Sweep] -> [Compact] -> Free memory -> Continue
```

### Разные сборщики мусора

Отлично! Очень мощная тема 🔥  
Расскажу, **как работают разные сборщики мусора (GC) в Java "под капотом"**,
максимально понятно и по делу 👇
...
✅ Умеет прогнозировать паузы (`-XX:MaxGCPauseMillis=200` — постарается
уложиться).
...
✅ Поддержка **очень больших куч** (от 4–8 ГБ до сотен ГБ).  
