...
Отлично! Давай посмотрим, как устроена внутренняя структура **`Map.Entry<K, V>`
**, которая представляет **одну пару "ключ–значение"** в любой реализации
интерфейса `Map`.
...
Хочешь, я покажу графически, как выглядит цепочка узлов `Entry` в одном бакете
при коллизии?
...
Хочешь, я покажу, как работает `get("C")` на основе этой цепочки?
...
Отлично! Давай реализуем **упрощённую версию `HashMap` вручную** — с минимальной
функциональностью:
...
## 🔧 Упрощённый `MyHashMap<K, V>` на Java:
...
### Как вставляется новый элемент при коллизии на шаги?

Хорошо! Давай пошагово разберём, **как вставляется элемент в `HashMap`**, когда
происходит **коллизия** — то есть два ключа попадают в один и тот же бакет.
