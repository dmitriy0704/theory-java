...
3. **Настройка `FetchType`**:
   ...
   **Настройка `fetch` вs аннотациях:** Вы можете настроить стратегию выборки с
   помощью аннотаций `@OneToMany`, `@ManyToOne` и т.д., указав
   `fetch = FetchType.EAGER` для загрузки связанных сущностей сразу.
...
5. **Пагинация:** Если у вас очень большой объем данных, рассмотрите возможность
   использования пагинации, чтобы уменьшить количество загружаемых данных за
   один раз.

6. **Оптимизация запросов:** Проверьте свои запросы на предмет возможности их
   оптимизации или использования более эффективных подходов к выборке данных.

Избегание проблемы N+1 требует внимательного проектирования структуры ваших
запросов и понимания того, как Hibernate управляет загрузкой связанных данных.

### Причины появления

Проблема N+1 в Hibernate возникает, когда для загрузки связанных сущностей
выполняется слишком много запросов к базе данных. Это происходит, когда вы
загружаете коллекцию или связанные объекты, и для каждого объекта выполняется
отдельный запрос. В результате вы получаете один запрос для основной сущности и
N дополнительных запросов для каждой связанной сущности, что может значительно
ухудшить производительность приложения.

### Пример проблемы N+1

Предположим, у вас есть две сущности: `Author` и `Book`, где один автор может
иметь много книг. Если вы загружаете список авторов и затем для каждого автора
загружаете его книги, это приведет к выполнению одного запроса для получения
всех авторов и N запросов для получения книг каждого автора.
...
#### Пример использования Entity Graphs
...
put("javax.persistence.fetchgraph",entityGraph);
...
> Отличие HQL - запросов от native запросов, JPQL, Criteria запросы
