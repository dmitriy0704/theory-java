...
Map<K,V> - принимает два дженерика типа K и V. Повторяющиеся элементы запрещены.
...
- **LinkedHashMap<K, V>** — образует дважды связанный список ключей. Этот
  механизм
  эффективен, только если превышен коэффициент загруженности карты при работе с
  кэш-памятью и др.
...
- **V put(K key, V value)** - добавляет ключ-значение. Если key существует -
  value перезаписывается.
